...
class myDateset(torch.utils.data.Dataset):
    def __init__(self, root, domain=0, mean=0, std=1, Max=0, Min=0, norm=True):
        ...
        print("Looking for files in:", self.root)
        print("Matched .mat files:", self.files)
        if len(self.files) == 0:
...
def CreateDataset(input_directories, domains=[0, 1, 2, 3, 4, 5, 6]):
    all_files = []  # 用于存储所有 .mat 文件的路径
    for g in range(1, 8):
        print(f"Checking g{g}: {os.path.join(test_directory, f'g{g}')}")

    # 遍历输入目录及其子目录
    for input_directory in input_directories:
        print(f"Searching in directory: {input_directory}")
        mat_files = glob.glob(input_directory + '/**/*.mat', recursive=True)
        all_files.extend(mat_files)

    if len(all_files) == 0:
        raise ValueError("No .mat files found in the specified directories.")

    # 根据找到的文件创建数据集
    dataset = None
    for i, file in enumerate(all_files):
        print(f"Processing file: {file}")
        # 创建 myDateset 实例，使用文件所在的目录作为输入
        cur_set = myDateset(os.path.dirname(file), domain=domains[i])  # 循环使用 domains
        data_mean, data_std, data_max, data_min = DataStatistic(cur_set)

        cur_set = myDateset(os.path.dirname(file), data_mean, data_std, data_max, data_min, norm=True)
    ...
    return dataset
def all_dataCreateDatasetIters(input_directory,batch_size=64,shuffle=True,num_workers=2,norm=False,requried_Val_iter=False,val_ratio=0.2):
    '''创建用于WMSUNN的迭代器'''
...
def DataStatistic(dataset):
    ...
    print(len(dataset))
    data_iter = Data.DataLoader(dataset, batch_size=256, num_workers=2, shuffle=False)
...
if __name__ == '__main__':
    ...
    test_directory = './datasets/CPSC'
    from itertools import cycle

    # 检查路径下的文件
    mat_files = glob.glob(test_directory + '/**/*.mat',recursive=True)
    print(f"Found {len(mat_files)} .mat files in {test_directory}")

    ds = CreateDataset([test_directory]) 
    testloader = Data.DataLoader(ds, batch_size=256, num_workers=2)
    t = cycle(testloader)
    for i in range(500):
